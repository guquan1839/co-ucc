...
//这有问题
template<typename Datatype>
__device__ inline uint8_t base_minmax_algo(Datatype input_value,float scale, float lower_bound, float upper_bound){
...
template<>
__device__ inline uint8_t base_minmax_algo<half>(half input_value,float scale,float lower_bound,float upper_bound){
...
template<>
__device__ inline uint8_t base_minmax_algo<float>(float input_value,float scale,float lower_bound,float upper_bound){
    float compressed_value  = input_value * scale;
    uint8_t offset = compressed_value - lower_bound;
    return offset;
}
...
template<typename Datatype>
__device__ inline Datatype base_minmax_decompress(uint8_t compressed_value,float scale, float lower_bound, Datatype placeholder){
    return (compressed_value + lower_bound) / scale;
}

template<>
__device__ inline half base_minmax_decompress<half>(uint8_t compressed_value,float scale, float lower_bound, half placeholder){
...
__global__ void compress_float2uint8_kernel(const void * input_arr, int input_chunk_count, void * output_arr, int output_chunk_count){
    ...
    for(int m = index_i; m < input_chunk_count; m += blockDim.x * gridDim.x){
        ...
        compress_arr[k] = base_minmax_algo(origin_arr[n], scale,lower_bound,upper_bound);
    }
...
__global__ void decompress_uint82float_kernel(const void* input_arr,int input_chunk_count, void * output_arr, int output_chunk_count){
    ...
    for(int m = index_i; m < input_chunk_count; m += blockDim.x * gridDim.x){
        ...
        decompressed_arr[n] = base_minmax_decompress(compressed_arr[k],scale,lower_bound,upper_bound,decompressed_arr[n]);
    }
...
__global__ void decompress_uint82float_reduce_kernel(const void* input_arr, int input_chunk_count, const void* reduce_input_arr, void *output_arr, int output_chunk_count){
    ...
    for(int m = index_i; m < output_chunk_count; m += blockDim.x * gridDim.x){
        int n = index_j * output_chunk_count + m;
        int k = index_j * input_chunk_count + 32 + m;
        decompress_arr[n] = reduce_buffer[n] + base_minmax_decompress(compress_arr[k], scale, lower_bound, upper_bound, decompress_arr[n]);
    }
}
